...
FLUX2_TRANSFORMER_KEYS_RENAME_DICT = {
    ...
    # Handle separately since we need to swap mod params
...
FLUX2_TRANSFORMER_DOUBLE_BLOCK_KEY_MAP = {
    ...
    # Handle fused QKV projections separately — break into
    # Q, K, V projections
...
class Flux2KleinDiffusionGGUFAdapter(DiffusionGGUFAdapter):
    ...
    def is_compatible(
        ...
        if model_class_name and model_class_name.startswith("Flux2"):
            return True
        return bool(model_type and model_type.lower().startswith("flux"))
    ...
    def weights_iterator(self) -> Iterable[tuple[str, torch.Tensor]]:
        def custom_weights_adapter(
            ...
            for name, weight in weights:
                # Handle adaLN modulation params that require swapping
                # shift and scale
